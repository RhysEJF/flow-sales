...
2. For each interaction, in time order:
   ...
   - Set `behaviour` to 1 only when the rep visibly applied the framework for that element in this interaction (asked the question, tested the champion, mapped the process, named the alternative), and list the matching `behaviourTags` from the framework's vocabulary for that element, applying the tag tests in the methodology skill's table "Behaviour tags: when each one applies" literally (attach every tag whose test is met; sibling tags such as identified-eb and asked-eb-access are different acts, not levels of one act). General tags (`secured-next-step`, `multi-threaded`, `summarised-and-confirmed`) may be attached to whichever element they served; `multi-threaded` applies whenever a second buyer-side person takes part. A buyer volunteering information without a rep question is evidence, not behaviour.
