...
If a team folder is set (`team.folder` in the status payload), run `fs.py team sync --pull-only --json` first and say how many judged deals came in from teammates. Run `fs.py plan-assessment --json` (pass through `--sample N`, `--force` and `--deal <id>` from `$ARGUMENTS`). The planner counts what is actually in the store after the pull and the link step, so its totals are real: deals, interactions, characters, the token estimate, `estCostUsd` and `skipped.reusedFromTeam` (deals the team had already judged on exactly these interactions, reused for free).

Print one line and carry on, no question. The line names the deals and interactions to judge, the deals reused from the team, the token figure and the dollar range at list price for the judge model, and says that on a Claude subscription it comes out of the plan's usage rather than a bill; for example `Judging 32 deals, 118 interactions, 0 reused from the team. About 1.2M tokens, $3 to $5 at Sonnet list price, plan usage on a subscription.` Say once that a rerun only judges deals whose interactions changed. If nothing was planned, say every deal is unchanged and skip to step 5.

Do not ask before running. The one exception is size: when the plan holds more than 40 deals, ask once with AskUserQuestion whether to run all of them or a sample of 10 deals first (recommended), or narrow the window (then set `window` with `fs.py config set` and re-plan). Below that, the run starts as soon as the line is printed.
