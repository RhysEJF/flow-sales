...
**Shared folder for judged deals, never on `--demo`.** Every deal FlowSales scores produces one judged file (the element levels, the buyer quotes behind them, the judge's reasons and next questions), and producing it costs model tokens. Teammates who run FlowSales on the same deals would each pay to produce the same file again, unless the files sit in a folder the team already syncs. That is all this step is. Run `fs.py team candidates --json`: it looks under the working folder (where a folder added to a Cowork session with "Add folder" appears) and, on a Mac, under the usual sync locations (Google Drive, OneDrive, Dropbox, Box), for a folder named FlowSales with an `assessments` folder inside, up to four folders deep. It only finds one when a teammate created it inside a shared folder this machine also syncs; it finds nothing in a cloud session.

- In a cloud session (the workspace is not the user's own machine) and nothing was found: ask nothing. Say in one line: "Sharing judged deals with teammates needs the synced folder added to this session with Add folder in the desktop app; skipping it changes nothing else." If something was found (the user added the folder), treat it like a folder found on a Mac.
- A rep (`me.role` rep) and a folder was found: "FlowSales found a folder your team uses to share judged deals: <path>. Using it means a deal a teammate has already scored is not scored again on your machine, which saves tokens. It holds those judged files only, never the calls or emails. Use it?" Options: yes; skip for now (safe). On yes, `fs.py team join <path>`.
- A rep and nothing was found: say nothing. There is nothing for them to set up; if the team shares a folder later, `/flow-sales:status` will offer it once it is visible.
- The team person (`me.role` team or both): "Each deal you score produces a judged file, and producing it costs tokens. Teammates who install FlowSales would score the same deals again on their own machines, unless the judged files sit in a folder your team already syncs (Google Drive, OneDrive, Dropbox). The folder holds those judged files only, never the calls or emails. Set that up now, or skip? Skipping is safe: everything works alone, and `/flow-sales:status` shows how to do it later." Options: skip for now (safe); create it, I will give you a path inside a folder my team syncs; use the one found at <path> (only when one was found). On create, `fs.py team init <path>`, then say: "Tell teammates the path. Their setup finds it on its own if their Drive syncs that folder; otherwise they paste it when asked." On use, `fs.py team join <path>`.

Then say exactly what happens next, in three lines, depending on who they are: a rep runs `/flow-sales:daily-sync` every morning (two minutes; it pulls their own deals, reuses anything the team has judged, judges the rest, then briefs them); the team person runs `/flow-sales:audit` once for the benchmark (state the rough duration from the interaction count and the cost the planner prints) and then weekly; everyone can run `/flow-sales:status` when in doubt. Every run is logged in `.flow-sales/runs.jsonl`; deleting `.flow-sales/` removes everything FlowSales knows on this machine, and the team folder keeps the judged deals.
