...
- Skills: setup, link, audit, daily-sync (was standup), retro, impact, status, coaching-voice, methodology. Agent: `agents/deal-assessor.md`. `claude --plugin-dir ~/flow-sales plugin details flow-sales` shows 8 skills, 1 agent, 2 MCP servers, about 740 always-on tokens.
...
Usability rounds (2026-09-08, branch `usability-fixes`): two synthetic-panel rounds and their fixes are recorded in `DESIGN-NOTES.md` (report round one, product round two: link folded into audit, install page around an environment check, `/flow-sales:status`, dollar estimate and `--estimate-only`, share-safe exports, consent questions in setup, tab renames). Skills count: setup, audit, daily-sync (renamed from standup on 2026-09-09), retro, impact, status, plus link as the standalone repair command.

## 5. What remains (in order)

0. First, two live checks that need Rhys: (a) sign in to the HubSpot connector once and run `/flow-sales:setup` on real data; the setup skill's connector walk saves replies into `.flow-sales/cache/hubspot-mcp/` and `fs.py import hubspot-cache` reads them; this is the first observation of the live reply shapes (`docs/hubspot.md`, Connector route). (b) The Cowork checklist in `docs/cowork.md`, fifteen minutes; line 2 (Python inside Cowork) decides everything.
...
2. Run `/flow-sales:daily-sync Tom Ellis` and `/flow-sales:retro Tom Ellis` in the demo project; fix the skills where the flow is clumsy (they have never been exercised end to end). Tom Ellis is the rep whose adoption rises after the training date.
...
5. Done 2026-09-08: `fs.py eval-golden build | compare` (CONTRACTS 13) plus a first `claude plugin eval` case in `evals/cases/` (unrun: `plugin eval` is early access on this account). Two Sonnet golden runs recorded in `evals/golden/runs.jsonl`: evidence targets met (exact 0.86 to 0.88, within one 0.98), tag agreement 0.80 against 0.90 after tag definitions were added. Next step is a human review of the disagreement snippets listed in `evals/golden/README.md`, ideally by Rhys or Tom Parker if he becomes the rubric reviewer. The golden scratch store is `~/flow-sales-golden/.flow-sales` (first-run assessments kept in `~/flow-sales-golden/run1-sonnet/`).
...
- Rubric hash: the behaviour tag definitions live in `skills/methodology/SKILL.md`, not in `frameworks/*.json`, so that adding them did not change the rubric hash and force a re-judge of the 32 demo deals. Move them into the framework files at the next rubric version bump, when a re-judge is planned anyway.
