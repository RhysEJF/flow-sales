...
| 4 | `/flow-sales:audit` | The link line, one cost line, no question (32 deals is under the 40-deal sample gate), then "Judged 1 of 32 ..." lines | **Subagents in parallel.** Audit launches the deal-assessor agent five at a time. Cowork supports subagents; the parallel launch is the thing to watch. If they run one at a time it still finishes, only slower. The agent pins Sonnet whatever the session model; pick Opus for the session |
